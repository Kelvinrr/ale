...
class FrameChain(nx.DiGraph):
    ...
    def from_spice(cls, sensor_frame, target_frame, center_ephemeris_time, 
        ...
        if exact_ck_times and not nadir:
            try:
                sensor_times = spiceql_call("extractExactCkTimes", {"observStart": ephemeris_times[0], 
                                                                    "observStop": ephemeris_times[-1], 
        ...
        asyncio.run(frame_chain.generate_time_dependent_rotiations(sensor_time_dependent_frames, sensor_times, mission))
        asyncio.run(frame_chain.generate_time_dependent_rotiations(target_time_dependent_frames, target_times, mission))
    ...
    async def generate_time_dependent_rotiations(self, frames, times, mission=""):
        """
        ...
        for i, frame in enumerate(frames):
            ...
            if len(avs) == 0:
                avs = None
            rotation = TimeDependentRotation(quats, times, frame[0], frame[1], av=avs)
